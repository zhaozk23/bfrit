...
impl Backpatch {
    fn new(addr: usize, src_index: usize, dst_index: usize) -> Self {
        Self {
            addr,
            src_index,
            dst_index,
...
fn jit_compile(ops: &[Op]) -> Code {
    ...
    for (i, op) in ops.iter().enumerate() {
        ...
        match op {
            ...
            Op::Jz(value) => {
                ...
                code.extend_from_slice(&[0x00, 0x00, 0x00, 0x00]); // placeholder for jump address
                backpatches.push(Backpatch::new(op_addr, i, *value));
            }
            Op::Jnz(value) => {
                code.extend_from_slice(&[0x48, 0x31, 0xC0]); // xor rax, rax
                code.extend_from_slice(&[0x8A, 0x07]); // mov al, byte ptr [rdi]
    ...
    for bp in backpatches {
        let src_addr = addrs[bp.src_index];
        let dst_addr = addrs[bp.dst_index];
        let rel32 = (dst_addr as isize - (src_addr as isize + 4)) as i32;
...
fn main() {
    ...
    while let Some(ch) = lexer.next() {
        match ch {
            ...
            b']' => {
                let addr = addr_stack.pop().unwrap_or_else(|| {
                    panic!("Unmatched closing bracket at position {}.\n", lexer.pos)
                });
                ops.push(Op::new(ch, addr));
                ops[addr] = Op::new(b'[', ops.len());
            }
            _ => panic!("Invalid character in Brainfuck code: {}", ch),
        }
    }
    // for (i, op) in ops.iter().enumerate() {
    //     println!("{}: {}", i, op);
